...
import { createUsuario, deactivateUsuario, deleteUsuarioFisico, existsUsuarioByCorreo, listUsuarios } from "@/data/models/usuarios.model";
...
app.get("/me", authMiddleware, async (c) => {
  try {
    ...
    console.error("[Administradores] Error al obtener usuario:", e);
...
app.get("/", async (c) => {
  try {
    ...
    console.error("[Administradores] Error al listar:", e);
...
app.post(
  ...
  async (c) => {
    try {
      ...
      if (!row) return c.json({ error: "Error al crear administrador" }, 500);

      return c.json({ ...row, codigo }, 201);
    } catch (e) {
      console.error("[Administradores] Error al crear:", e);
...
app.patch(
  ...
  async (c) => {
    try {
      ...
      `;
      if (!row) return c.json({ error: "Administrador no encontrado" }, 404);
      return c.json(row);
    } catch (e) {
      console.error("[Administradores] Error al actualizar:", e);
      return c.json({ error: "Error al actualizar administrador" }, 500);
    }
  }
);
...
app.delete(
  ...
  async (c) => {
    try {
      ...
      if (!row) return c.json({ error: "Administrador no encontrado" }, 404);
      return c.json({ message: "Administrador desactivado" });
    } catch (e) {
      console.error("[Administradores] Error al desactivar:", e);
      return c.json({ error: "Error al desactivar administrador" }, 500);
    }
  }
);
